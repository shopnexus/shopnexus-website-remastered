...
export function OrderTracking({ order }: OrderTrackingProps) {
  ...
  const trackingSteps = [
    ...
    },
    {
      ...
      date: order.status === "shipped" || order.status === "delivered" ? "2024-01-17" : undefined,
    },
    {
      id: "delivered",
      title: "Delivered",
                ...
                <div key={step.id} className="flex space-x-4">
                  <div className="flex flex-col items-center">
                    ...
                    {index < trackingSteps.length - 1 && (
                      <div className={`mt-2 h-8 w-0.5 ${step.completed ? "bg-primary" : "bg-muted"}`} />
                    )}
                  </div>
                  <div className="flex-1 pb-8">
                    <div className="flex items-center justify-between">
                      <h4
                        className={`font-medium ${step.completed || isActive ? "text-foreground" : "text-muted-foreground"}`}
                      >
                        {step.title}
                      </h4>
                      {step.date && (
                    ...
                    <p
                      className={`text-sm ${step.completed || isActive ? "text-muted-foreground" : "text-muted-foreground/60"}`}
                    >
                      {step.description}
                    </p>
                  </div>
